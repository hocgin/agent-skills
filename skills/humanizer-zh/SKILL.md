...
如果用户提供了一段写作样本（他们之前写的内容），请在重写前对其进行分析：
...
2. **在重写中匹配他们的语气。** 不要只是去除 AI 模式——要用样本中的模式替换它们。如果他们写短句，你就不要生成长句。如果他们用“东西”和“玩意儿”，你就不要升级成“元素”和“组件”。

3. **如果没有提供样本，** 则退回到默认行为（参考下面“个性与灵魂”部分中自然、多变、有观点的语气）。

### 如何提供样本
- 行内提供：“帮我把这段文本改得更自然。这里有一段我平时的写作样本供语气参考：[样本]”
- 文件提供：“帮我把这段文本改得更自然。请参考 [文件路径] 中的写作风格。”
